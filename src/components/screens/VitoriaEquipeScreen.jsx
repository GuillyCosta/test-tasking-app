// src/components/screens/VitoriaEquipeScreen.jsx
'use client';
...
export function VitoriaEquipeScreen() {
  const {
    goToStep,
    players,
  ...
  const leais = players.filter(p => !p.isImpostor);
  ...
  const heroiDaPartida = crownFoundResult?.playerName || "Um bravo membro da equipe";

  // Mensagem de Vitória
  let mensagemVitoria = `Parabéns, Equipe Leal! A Coroa foi recuperada graças a ${heroiDaPartida}!`;
  if (crownFoundResult?.playerName && players.find(p => p.name === crownFoundResult.playerName)?.isImpostor) {
    ...
    mensagemVitoria = `Atenção! ${crownFoundResult.playerName} (um Impostor) tentou manipular a Coroa, mas a Equipe Leal prevaleceu!`;
  }
